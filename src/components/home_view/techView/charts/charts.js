...
const StrokeGradient = (props) =>{
    ...
    return(
        <defs>
            <linearGradient id={props.id} x1={posLineGrad.x1} y1={posLineGrad.y1} x2={posLineGrad.x2} y2={posLineGrad.y2}>
                {listColors.map(item => <stop offset={item.offset+'%'} stopColor={'#'+item.color} key={item.color}/>)}
            </linearGradient>
        </defs>
    );
};