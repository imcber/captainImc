...
const contactIllus = '/images/flame-no-comments.png';
...
const ContactView = () => {
    ...
    return (
        <SectionContainer
            id={'contactView'}
            title={getDataLeg('lg.menu.contact.title')}
            style={{ display: 'flex' }}
            classHeader={'header-contact-section'}
            classBody={'body-contact-section'}
        >
            <ContactContent />
        </SectionContainer>
    );
        ...
        return (
            <li onClick={handlerClick}>
                <IconContext.Provider
                    value={{ color: '#3AAFA9', size: '4vw', className: 'social-item' }}
                >
                    {thisItem.icon}
                </IconContext.Provider>
            </li>
