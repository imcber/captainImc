...
const beraviImg = '/images/sites/Beravi.jpg';
const cthcImg = '/images/sites/captain.jpg';
const leafireImg = '/images/sites/leafire.jpg';
const listDataSites = {
    beravi: {
        img: beraviImg,
...
const WorkView = () => {
    ...
    return (
        <SectionContainer
            id={'workView'}
            title={getDataLeg('lg.menu.work.title')}
            style={{ display: 'flex' }}
            classHeader={'header-work-section'}
            classBody={'body-work-section'}
        >
            <WorkContent actualWorkDisp={actualWorkDisp} />
            <ListSitesSection
                listPagination={listPagination}
                numList={numListDisplay}
...
const ListSitesSection = (props) => {
    ...
    const NavigationButton = (props) => {
        const navDirection =
            props.direction === 'prev' ? <IoIosArrowBack /> : <IoIosArrowForward />;
    ...
    const navigatePagination = (position) => {
        ...
        newNumPag =
            newNumPag >= listPagination.length
                ? 0
                : newNumPag < 0
                ? listPagination.length - 1
                : newNumPag;
    ...
    return (
        <div className={'carou-work'}>
            <div className={'list-site-section'}>
                ...
                {listPagination.length > 1 && (
                    <NavigationButton direction={'next'} style={{ right: '0' }} />
                )}
                <div className={'list-site-section-container'} ref={listSitesRef}>
                    {listDisplay.map((item) => (
                        <CardSite item={item} key={item.name.toUpperCase()} />
                    ))}
                </div>
            </div>
        </div>
...
const WorkContent = (props) => {
    ...
    const BigViewSite = () => {
        ...
        const ContainerInView = (props) => {
            ...
            return <div className={objClass}>{props.children}</div>;
        };
            ...
            <div className={'big-view-site'}>
                ...
                <ContainerInView class={'container-btn-go'}>
                    <div className={'big-view-desc-container'}>
                        ...
                        <button className='btn-explore' onClick={goSite}>
                            {getDataLeg('lg.menu.work.go')}
                        </button>
                    </div>
                </ContainerInView>
            </div>
