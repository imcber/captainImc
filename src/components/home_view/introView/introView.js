...
const IntroView = () => {
    ...
    const ComplementView = (props) => {
        return <div className={'complement-intro'}>{props.children}</div>;
    };

    return <IntroViewReturn />;
};
...
function HeaderIntro(props) {
    ...
    const LanguageItem = (props) => {
        ...
        return (
            <span
                onClick={indLang ? switchLanguage : () => {}}
                style={!indLang ? { color: '#DEF2F1' } : {}}
            >
    ...
    const HeaderIntro = (props) => {
        return (
            <div className={props.class1}>
                <div className={props.class2}>{props.children}</div>
            </div>
        );
    };
